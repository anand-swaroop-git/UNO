...
aws_http_80_listener_arn=`aws elbv2 describe-listeners --load-balancer-arn $alb_arn --query 'Listeners[].[ListenerArn, Port]' --output text | grep 80 | awk '{print $1}'`
echo "Found."
echo "Finding Target Groups..."
create_tg_arn=$(aws elbv2 describe-target-groups --names uno-application-lb-tg-create --query 'TargetGroups[].TargetGroupArn' --output text)
read_tg_arn=$(aws elbv2 describe-target-groups --names uno-application-lb-tg-read --query 'TargetGroups[].TargetGroupArn' --output text)
update_tg_arn=$(aws elbv2 describe-target-groups --names uno-application-lb-tg-update --query 'TargetGroups[].TargetGroupArn' --output text)
echo "Found."
echo
...
aws elbv2 create-rule \
    ...
    --actions Type=forward,TargetGroupArn=$create_tg_arn &> /dev/null
echo "Registered."
...
aws elbv2 create-rule \
    ...
    --actions Type=forward,TargetGroupArn=$read_tg_arn &> /dev/null
echo "Registered."
...
aws elbv2 create-rule \
    ...
    --actions Type=forward,TargetGroupArn=$update_tg_arn &> /dev/null
echo "Registered."
echo
